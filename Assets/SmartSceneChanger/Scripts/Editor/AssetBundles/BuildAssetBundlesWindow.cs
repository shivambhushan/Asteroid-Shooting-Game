...
namespace SSC
{
    ...
    public class BuildAssetBundlesWindow : EditorWindow
    {
        ...
        public class EncryptionInfo
        {
            ...
            /// Crypto version
            /// </summary>
            public CryptoVersion cryptoVersion = CryptoVersion.Ver1_depricated;
            ...
            public void validate()
            {
                ...
                if (this.tempAssetBundlesFolderPath.EndsWith("/", StringComparison.Ordinal))
                {
                    this.tempAssetBundlesFolderPath = this.tempAssetBundlesFolderPath.Remove(this.tempAssetBundlesFolderPath.Length - 1);
                }

                if (this.tempEncryptedFolderPath.EndsWith("/", StringComparison.Ordinal))
                {
                    this.tempEncryptedFolderPath = this.tempEncryptedFolderPath.Remove(this.tempEncryptedFolderPath.Length - 1);
                }

                if (this.tempAssetBundlesFolderPath != "Assets" && !this.tempAssetBundlesFolderPath.StartsWith("Assets/", StringComparison.Ordinal))
                {
                    this.tempAssetBundlesFolderPath = "Assets/" + this.tempAssetBundlesFolderPath;
                }

                if (this.tempEncryptedFolderPath != "Assets" && !this.tempEncryptedFolderPath.StartsWith("Assets/", StringComparison.Ordinal))
                {
        ...
        protected bool createEncryptedFiles()
        {
            ...
            try
            {
                ...
                foreach (string file in Directory.GetFiles(m_encryptionInfo.tempAssetBundlesFolderPath, "*", SearchOption.AllDirectories))
                {
                    ...
                    if (file.EndsWith(".meta", StringComparison.Ordinal) || file.EndsWith(".manifest", StringComparison.Ordinal))
                    {
                        continue;
                    }
        ...
        protected Dictionary<string, LabelAndVariant> setFakeAssetBundleInfos()
        {
            ...
            foreach (string path in paths)
            {
                ...
                if (ai && !string.IsNullOrEmpty(ai.assetBundleVariant))
                {
        ...
        protected bool createEncryptedFile(string originFilePath)
        {
            ...
            try
            {
                ...
                if (data.Length > 0)
                {
                    ...
                    assetBundleName = this.m_encryptionInfo.encryptedAssetBundlePrefix + assetBundleName.Remove(0, assetBundleName.IndexOf("/", StringComparison.Ordinal) + 1).ToLower();
            ...
            {
                Debug.LogErrorFormat("Exception : {0} : {1}", originFilePath, e.Message);
            }
        ...
        protected bool buildAssetBundles(string buildPath, BuildTarget target, string manifestName, List<AssetBundleBuild> builds = null)
        {
            ...
            // buildPath
            {
                ...
                buildPath += buildPath.EndsWith("/", StringComparison.Ordinal) ? "" : "/";
